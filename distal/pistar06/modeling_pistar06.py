    ...
    def extra_repr(self) -> str:
        if self.dense is not None:
            return (
                ...
                f"adaptive=True, cond_dim={self.cond_dim}"
            )
    ...
    class _PiGemmaDecoderLayerBase(GradientCheckpointingLayer):
        ...
        def forward(
            ...
            **kwargs,
            ...
            hidden_states = _gated_residual(residual, hidden_states, gate)  # ty:ignore[invalid-assignment]
...
class PiGemmaModelDirect(nn.Module):
    ...
    def __init__(self, config: GemmaConfig):
        ...
        self.embed_tokens = GemmaTextScaledWordEmbedding(
            config.vocab_size,
            config.hidden_size,
            self.padding_idx,  # ty:ignore[invalid-argument-type]
    ...
    def forward(
        ...
        **kwargs,
        ...
        if position_ids is None:
            position_ids = cache_position.unsqueeze(0)  # ty:ignore[invalid-assignment, unresolved-attribute]
        ...
        if (
            len(self.layers) > 0
            and self.layers[0].self_attn.q_proj.weight.dtype == torch.bfloat16  # ty:ignore[unresolved-attribute]
        ):
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:  # ty:ignore[not-iterable]
            if output_hidden_states:
                all_hidden_states += (hidden_states,)  # ty:ignore[unsupported-operator]
            ...
            if output_attentions:
                all_self_attns += (layer_outputs[1],)  # ty:ignore[unsupported-operator]
        ...
        return BaseModelOutputWithPast(
            ...
            hidden_states=all_hidden_states,  # ty:ignore[invalid-argument-type]
...
def create_sinusoidal_pos_embedding(
    ...
    dtype = get_safe_dtype(torch.float64, device.type)  # ty:ignore[unresolved-attribute]
...
def compute_layer_complete(
    ...
    for i, hidden_states in enumerate(inputs_embeds):
        ...
        out_emb, gate = layernorm_forward(
            ...
            out_emb,  # ty: ignore[invalid-argument-type]
            adarms_cond[i],
        )
...
class PaliGemmaWithExpertModel(nn.Module):
    ...
    def __init__(
        ...
        action_expert_config_hf = CONFIG_MAPPING["gemma"](
            head_dim=action_expert_config.head_dim,  # ty:ignore[unknown-argument]
            hidden_size=action_expert_config.width,  # ty:ignore[unknown-argument]
            intermediate_size=action_expert_config.mlp_dim,  # ty:ignore[unknown-argument]
            num_attention_heads=action_expert_config.num_heads,  # ty:ignore[unknown-argument]
            num_hidden_layers=action_expert_config.depth,  # ty:ignore[unknown-argument]
            num_key_value_heads=action_expert_config.num_kv_heads,  # ty:ignore[unknown-argument]
            vocab_size=257152,  # ty:ignore[unknown-argument]
            hidden_activation="gelu_pytorch_tanh",  # ty:ignore[unknown-argument]
            dtype=precision,
            use_adarms=use_adarms[1],  # ty:ignore[unknown-argument]
            adarms_cond_dim=(action_expert_config.width if use_adarms[1] else None),  # ty:ignore[unknown-argument]
        )
        ...
        self.gemma_expert = ActionExpertDirect(config=action_expert_config_hf)  # ty:ignore[invalid-argument-type]
        _log_mem("PaliGemmaWithExpert: after gemma_expert init")
        self.gemma_expert.model.embed_tokens = None  # ty:ignore[invalid-assignment]
    ...
    def forward(
        ...
        if adarms_cond is None:
            adarms_cond = [None, None]  # ty:ignore[invalid-assignment]
        if inputs_embeds[1] is None:  # ty:ignore[not-subscriptable]
            prefix_output = self.paligemma.model.language_model.forward(
                inputs_embeds=inputs_embeds[0],  # ty:ignore[not-subscriptable]
                attention_mask=attention_mask,
                position_ids=position_ids,
            ...
            suffix_output = None
        elif inputs_embeds[0] is None:  # ty:ignore[not-subscriptable]
            suffix_output = self.gemma_expert.model.forward(
                inputs_embeds=inputs_embeds[1],  # ty:ignore[not-subscriptable]
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_values=past_key_values,  # ty:ignore[invalid-argument-type]
                use_cache=use_cache,
            ...
            for layer_idx in range(num_layers):
                if use_gradient_checkpointing:
                    inputs_embeds = torch.utils.checkpoint.checkpoint(  # ty:ignore[possibly-missing-submodule]
            ...
            if use_gradient_checkpointing:
                outputs_embeds = torch.utils.checkpoint.checkpoint(  # ty:ignore[possibly-missing-submodule]
                    compute_final_norms,
                    inputs_embeds,
                    adarms_cond,
                    use_reentrant=False,
...
class PiStar06Pytorch(nn.Module):
    ...
    def __init__(
        ...
        self.paligemma_with_expert = PaliGemmaWithExpertModel(
            ...
            precision=config.dtype,  # ty:ignore[invalid-argument-type]
        ...
        if config.compile_model:
            ...
            self.sample_actions = torch.compile(  # ty:ignore[invalid-assignment]
                self.sample_actions, mode=config.compile_mode
            )
            self.forward = torch.compile(self.forward, mode=config.compile_mode)  # ty:ignore[invalid-assignment]

    def gradient_checkpointing_enable(self):
        """Enable gradient checkpointing for memory optimization."""
        self.gradient_checkpointing_enabled = True
        paligemma_model = self.paligemma_with_expert.paligemma.model
    ...
    def _apply_checkpoint(self, func, *args, **kwargs):
        ...
        if self.gradient_checkpointing_enabled and self.training:
            return torch.utils.checkpoint.checkpoint(  # ty:ignore[possibly-missing-submodule]
                func,
                *args,
                use_reentrant=False,
                preserve_rng_state=False,
    ...
    def forward(
        ...
        if (
            self.paligemma_with_expert.paligemma.model.language_model.layers[
                0
            ].self_attn.q_proj.weight.dtype  # ty:ignore[unresolved-attribute]
            == torch.bfloat16
        ):
        ...
        def forward_func(
            ...
            (_, suffix_out), _ = self.paligemma_with_expert.forward(
                ...
                use_cache=False,
                adarms_cond=[None, adarms_cond],  # ty:ignore[invalid-argument-type]
            )
            return suffix_out
        ...
        **kwargs: Unpack[ActionSelectKwargs],
        ...
        for step in range(num_steps):
            ...
            if self._rtc_enabled():
                ...
                v_t = self.rtc_processor.denoise_step(  # ty:ignore[unresolved-attribute]
                    x_t=x_t,
    ...
    def denoise_step(
        ...
        outputs_embeds, _ = self.paligemma_with_expert.forward(
            ...
            position_ids=position_ids,  # ty:ignore[invalid-argument-type]
            past_key_values=past_key_values,
            inputs_embeds=[None, suffix_embs],  # ty:ignore[invalid-argument-type]
            use_cache=False,
            adarms_cond=[None, adarms_cond],  # ty:ignore[invalid-argument-type]
        )
...
class PiStar06Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if config.num_expert_layers > 0:
            ...
            expert_model.layers = expert_model.layers[: config.num_expert_layers]  # ty:ignore[invalid-assignment]
            logging.info(
        ...
        **kwargs,
        ...
        try:
            ...
            try:
                ...
                original_state_dict = load_file(resolved_file)  # ty:ignore[invalid-argument-type]
            ...
            if unexpected_keys:
                print(
                    "Unexpected keys when loading state dict: "
    ...
    def _setup_advantage_tokens(self) -> None:
        ...
        pos_ids = tokenizer.encode(" Advantage: positive", add_special_tokens=False)  # ty:ignore[unresolved-attribute]
        neg_ids = tokenizer.encode(" Advantage: negative", add_special_tokens=False)  # ty:ignore[unresolved-attribute]
    ...
    def _inject_advantage_text(
        ...
        for i in range(tokens.shape[0]):
            ...
            n_adv = adv_ids.shape[0]  # ty:ignore[not-subscriptable]

            if content_len + n_adv > seq_len:
                n_adv = seq_len - content_len
                if n_adv <= 0:
                    continue
                adv_ids = adv_ids[:n_adv]  # ty:ignore[not-subscriptable]

            tokens[i, content_len : content_len + n_adv] = adv_ids  # ty:ignore[invalid-assignment]
    ...
    def _forward_with_advantage(
        ...
        (_, suffix_out), _ = self.model.paligemma_with_expert.forward(
            ...
            position_ids=position_ids,  # ty:ignore[invalid-argument-type]
            past_key_values=None,
            inputs_embeds=[prefix_embs, suffix_embs],  # ty:ignore[invalid-argument-type]
            use_cache=False,
            adarms_cond=[None, adarms_cond],  # ty:ignore[invalid-argument-type]
        )
    ...
    def forward(
        ...
        if self.training:
            ...
            if self._train_step_count % 10 == 1:
                ...
                parts.append(
                    f"text=pos:{n_positive}/{n_total} drop:{n_dropped}/{n_total}"
                )
    ...
    @torch.no_grad()
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:  # ty:ignore[invalid-method-override]
    ...
    def _predict_action_chunk_cfg(
        ...
        **kwargs: Unpack[ActionSelectKwargs],
        ...
        _, past_kv_cond = self.model.paligemma_with_expert.forward(
            ...
            position_ids=pos_ids_c,  # ty:ignore[invalid-argument-type]
            past_key_values=None,
            inputs_embeds=[prefix_embs_c, None],  # ty:ignore[invalid-argument-type]
            use_cache=True,
        )
        ...
        _, past_kv_uncond = self.model.paligemma_with_expert.forward(
            ...
            position_ids=pos_ids_u,  # ty:ignore[invalid-argument-type]
            past_key_values=None,
            inputs_embeds=[prefix_embs_u, None],  # ty:ignore[invalid-argument-type]
            use_cache=True,
        )
    ...
    def _get_default_peft_targets(self) -> dict[str, any]:  # ty:ignore[invalid-type-form]
